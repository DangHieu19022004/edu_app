...
def check_chatbot_rate_limit(user_id):
    ...
    if now_ts - rate_limit.short_term_window_start >= SHORT_TERM_WINDOW_SECONDS:
        rate_limit.short_term_count = 0
        rate_limit.short_term_window_start = now_ts
    ...
    if rate_limit.short_term_count >= SHORT_TERM_LIMIT:
        rate_limit.updated_at = now_ts
        rate_limit.save()
        retry_after = max(1, SHORT_TERM_WINDOW_SECONDS - (now_ts - rate_limit.short_term_window_start))
        _log_spam(
            user_id=user_id,
            action="CHATBOT_SHORT_LIMIT",
            message_count=rate_limit.short_term_count,
            window="60s",
        )
        return False, RATE_LIMIT_ERROR_MESSAGE, retry_after

    rate_limit.short_term_count += 1
